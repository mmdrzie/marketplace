...
beforeAll(async () => {
  ...
  const listingResult = await db.query(
    ...
    [SELLER_ID, categoryId, cityRow.province_id, cityRow.city_id, 'Test Listing for Chat', 'test-listing-chat-' + Date.now(), 'Description', 1000000, 'fixed', 'published', 1, modelId],
  );
