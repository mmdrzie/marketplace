...
const { conversation: convRepo, message: msgRepo } = di.repos;
...
router.get('/unread-count', auth(), async (c) => {
  const count = await msgRepo.getUnreadCount(c.get('user').id);
...
router.get('/:id', auth(), async (c) => {
  ...
  return c.json({ data: { ...row, messages: result.messages.map(m => m.snapshot()) } });
});
...
router.post('/', auth(), rateLimiter('conversation:create'), zValidator('json', startSchema), async (c) => {
  ...
  if (listing.user_id === user.id) throw AppError.validation('Cannot start conversation with yourself');

  const dto = await createConversation.execute(new CreateConversationCommand(body.listing_id, user.id, listing.user_id as string));
  await sendMessage.execute(new SendMessageCommand(dto.id, user.id, body.message));
  const row = await findDetailedById(dto.id);
  const msgs = await msgRepo.findByConversation(dto.id);
  return c.json({ data: { ...row, messages: msgs.messages.map(m => m.snapshot()) } }, 201);
});
...
export { router as conversationV2Router };
